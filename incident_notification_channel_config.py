...
Canonical source: ``incident_notification_channel_settings`` (managed by the
admin dashboard, webhooks stored AES-256-GCM encrypted). The encryption
contract mirrors ``lib/instagram-dashboard/incident-notification-settings.ts``
in the backend: base64(iv[12] | tag[16] | ciphertext), key =
sha256(INCIDENT_NOTIFICATION_WEBHOOK_ENCRYPTION_KEY or
sha256hex("local-incident-notif:" + SUPABASE_SERVICE_ROLE_KEY)).

Env webhook URLs are only a legacy fallback and are disabled by default.
No webhook value ever reaches logs; callers must never log ``webhook_url``.
"""
...
def resolve_effective_channel_config(channel: str) -> dict[str, Any]:
    """Resolve one channel's effective delivery config (never logged)."""
    normalized = str(channel or "").strip().lower()
    base = {
        "channel": normalized,
        "row_present": False,

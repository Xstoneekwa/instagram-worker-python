...
TARGET_FOLLOWERS_RESUME_V2_SHADOW_ACCOUNT_IDS = _env_str(
    "TARGET_FOLLOWERS_RESUME_V2_SHADOW_ACCOUNT_IDS", ""
)
TARGET_FOLLOWERS_RESUME_V2_ENFORCE_ENABLED = _env_bool(
    "TARGET_FOLLOWERS_RESUME_V2_ENFORCE_ENABLED", False
)
TARGET_FOLLOWERS_RESUME_V2_HMAC_SECRET = _env_str(
    "TARGET_FOLLOWERS_RESUME_V2_HMAC_SECRET", ""
)
...
# Run Control dispatcher (opt-in; Play stays disabled until end-to-end validated).
RUN_CONTROL_DISPATCHER_ENABLED = _env_bool("RUN_CONTROL_DISPATCHER_ENABLED", False)
RUN_CONTROL_DISPATCHER_HEALTH_ONLY = _env_bool("RUN_CONTROL_DISPATCHER_HEALTH_ONLY", True)
RUN_CONTROL_DISPATCHER_LAUNCH_ENABLED = _env_bool("RUN_CONTROL_DISPATCHER_LAUNCH_ENABLED", False)
RUN_CONTROL_DISPATCHER_WORKER_ID = _env_str("RUN_CONTROL_DISPATCHER_WORKER_ID", "")
RUN_CONTROL_DISPATCHER_POLL_SECONDS = _env_float("RUN_CONTROL_DISPATCHER_POLL_SECONDS", 5.0)
RUN_CONTROL_DISPATCHER_LEASE_SECONDS = _env_int("RUN_CONTROL_DISPATCHER_LEASE_SECONDS", 120)
...
RUN_CONTROL_DISPATCHER_SUBPROCESS_TIMEOUT_SECONDS = _env_int(
    "RUN_CONTROL_DISPATCHER_SUBPROCESS_TIMEOUT_SECONDS",
    7200,
)
RUN_CONTROL_DISPATCHER_REQUIRE_ASSIGNMENT = _env_bool("RUN_CONTROL_DISPATCHER_REQUIRE_ASSIGNMENT", False)
RUN_CONTROL_DISPATCHER_ENFORCE_ASSIGNMENT_WINDOW = _env_bool(
    "RUN_CONTROL_DISPATCHER_ENFORCE_ASSIGNMENT_WINDOW",
    False,
)
RUN_CONTROL_DISPATCHER_ALLOW_EXISTING_QUEUE = _env_bool(
    "RUN_CONTROL_DISPATCHER_ALLOW_EXISTING_QUEUE",
    False,
)
RUN_CONTROL_CANCELLATION_CHECK_ENABLED = _env_bool("RUN_CONTROL_CANCELLATION_CHECK_ENABLED", False)
RUN_CONTROL_CANCELLATION_CHECK_INTERVAL_SECONDS = _env_float(
    "RUN_CONTROL_CANCELLATION_CHECK_INTERVAL_SECONDS",
    5.0,
)
...
# Login challenge dashboard actions (email verification / human review) are fail-open.
LOGIN_CHALLENGE_DASHBOARD_ACTION_ENABLED = _env_bool("LOGIN_CHALLENGE_DASHBOARD_ACTION_ENABLED", True)
LOGIN_CHALLENGE_DASHBOARD_ACTION_FAIL_OPEN = _env_bool("LOGIN_CHALLENGE_DASHBOARD_ACTION_FAIL_OPEN", True)
...
# P1b multi-target rotation — conservative test-safe defaults only.
# These values validate rotation mechanics (exhaustion switch, per-target budget,
# global cap respect). They are NOT production-tuned values.
# TODO(ops): tune production values after controlled multi-target runs (P2) prove
# selection, switch-on-exhaustion/budget, target_id attribution, and no mono-target regression.
# Example production reflection (not applied here): per-target budget ~30, max targets ~4,
# possibly varying by package (Growth / Pro / Premium). Do not raise defaults to 30/4
# before P2 controlled runs. Override via env for ops/test:
#   FOLLOW_TARGET_ROTATION_MAX_TARGETS_PER_RUN
#   FOLLOW_TARGET_MAX_FOLLOWS_PER_TARGET_PER_RUN
FOLLOW_TARGET_ROTATION_MAX_TARGETS_PER_RUN_DEFAULT = 3
FOLLOW_TARGET_ROTATION_MAX_TARGETS_PER_RUN_UPPER_BOUND = 10
FOLLOW_TARGET_MAX_FOLLOWS_PER_TARGET_PER_RUN_DEFAULT = 2
FOLLOW_TARGET_MAX_FOLLOWS_PER_TARGET_PER_RUN_UPPER_BOUND = 50
FOLLOW_TARGET_ROTATION_MAX_TARGETS_PER_RUN = max(
    1,
    min(
        _env_int(
            "FOLLOW_TARGET_ROTATION_MAX_TARGETS_PER_RUN",
            FOLLOW_TARGET_ROTATION_MAX_TARGETS_PER_RUN_DEFAULT,
        ),
        FOLLOW_TARGET_ROTATION_MAX_TARGETS_PER_RUN_UPPER_BOUND,
    ),
)
FOLLOW_TARGET_MAX_FOLLOWS_PER_TARGET_PER_RUN = max(
    1,
    min(
        _env_int(
            "FOLLOW_TARGET_MAX_FOLLOWS_PER_TARGET_PER_RUN",
            FOLLOW_TARGET_MAX_FOLLOWS_PER_TARGET_PER_RUN_DEFAULT,
        ),
        FOLLOW_TARGET_MAX_FOLLOWS_PER_TARGET_PER_RUN_UPPER_BOUND,
    ),
)
...
POST_FOLLOW_LIKE_STRICT_GRID_PROOF_MODE = _env_str(
...
# Historical 2026-05-23 JSONL: 25 verified actions, p90=14.541 s, rounded up.
UNFOLLOW_CONSERVATIVE_ACTION_SECONDS = max(1, _env_int("UNFOLLOW_CONSERVATIVE_ACTION_SECONDS", 15))
UNFOLLOW_FINALIZATION_RESERVE_SECONDS = max(0, _env_int("UNFOLLOW_FINALIZATION_RESERVE_SECONDS", 30))
...
AUTO_RESTART_DELAY_MINUTES = max(0, _env_int("AUTO_RESTART_DELAY_MINUTES", 10))
AUTO_RESTART_MAX_ATTEMPTS_PER_SESSION = max(
    0,
    _env_int("AUTO_RESTART_MAX_ATTEMPTS_PER_SESSION", 2),
)
AUTO_RESTART_MAX_RETRIES_AFTER_INITIAL_FAILURE = max(
    0,
    _env_int("AUTO_RESTART_MAX_RETRIES_AFTER_INITIAL_FAILURE", 2),
)
...
WELCOME_SCAN_FOLLOWERS_OPEN_WAIT_S = _env_float("WELCOME_SCAN_FOLLOWERS_OPEN_WAIT_S", 0.6)

# --- V4.2-B Welcome scan producer (enqueue only, no DM send) ---
WELCOME_SCAN_MAX_SCROLLS_V1 = 3
...
WELCOME_SCAN_CANDIDATE_ATTEMPT_CAP = _env_int("WELCOME_SCAN_CANDIDATE_ATTEMPT_CAP", 3)
...
# Unfollow budgets are planned once at the Follow -> Unfollow handoff.  The
# six-hour fallback is used only when the scheduler supplied no real deadline.
UNFOLLOW_FALLBACK_SESSION_SECONDS = max(
    900, _env_int("UNFOLLOW_FALLBACK_SESSION_SECONDS", 6 * 60 * 60)
)
UNFOLLOW_RECOVERY_RESERVE_SECONDS = max(
    0, _env_int("UNFOLLOW_RECOVERY_RESERVE_SECONDS", 75)
)
UNFOLLOW_NAVIGATION_RESERVE_SECONDS = max(
    0, _env_int("UNFOLLOW_NAVIGATION_RESERVE_SECONDS", 30)
)
UNFOLLOW_OUTREACH_RESERVE_SECONDS = max(
    0, _env_int("UNFOLLOW_OUTREACH_RESERVE_SECONDS", 5 * 60)
)
UNFOLLOW_DEADLINE_CHECK_INTERVAL_SECONDS = max(
    30, _env_int("UNFOLLOW_DEADLINE_CHECK_INTERVAL_SECONDS", 5 * 60)
)
UNFOLLOW_DEADLINE_CHECK_ACTION_INTERVAL = max(
    1, _env_int("UNFOLLOW_DEADLINE_CHECK_ACTION_INTERVAL", 25)
)
ACCOUNT_SESSION_OUTREACH_MINIMUM_SECONDS = max(
    30, _env_int("ACCOUNT_SESSION_OUTREACH_MINIMUM_SECONDS", 5 * 60)
)
...
# Live View host agent (LV-Web-2A) — disabled by default; manual launch only.
LIVE_VIEW_HOST_AGENT_ENABLED = _env_bool("LIVE_VIEW_HOST_AGENT_ENABLED", False)
LIVE_VIEW_HOST_ID = _env_str("LIVE_VIEW_HOST_ID", "")
LIVE_VIEW_POLL_INTERVAL_SECONDS = _env_float("LIVE_VIEW_POLL_INTERVAL_SECONDS", 2.0)
LIVE_VIEW_MAX_SESSION_SECONDS = _env_int("LIVE_VIEW_MAX_SESSION_SECONDS", 600)
LIVE_VIEW_FRAME_DIR = _env_str("LIVE_VIEW_FRAME_DIR", ".local/live-view-frames")
LIVE_VIEW_FRAME_UPLOAD_ENABLED = _env_bool("LIVE_VIEW_FRAME_UPLOAD_ENABLED", True)
LIVE_VIEW_FRAME_STORAGE_BUCKET = _env_str("LIVE_VIEW_FRAME_STORAGE_BUCKET", "live-view-frames")

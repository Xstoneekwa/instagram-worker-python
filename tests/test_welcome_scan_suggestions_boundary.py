...
class WelcomeScanSuggestionsBoundaryTest(unittest.TestCase):
    ...
    def test_loading_boundary_without_jobs_recovers_one_stable_screen(self) -> None:
        ...
        summary = scan.get_last_welcome_scan_summary()
        self.assertEqual(code, 0)
        self.assertEqual(summary["stop_reason"], "instagram_list_ambiguous_surface_recovered")
        self.assertEqual(summary["jobs_enqueued_count"], 0)
        self.assertFalse(summary["followers_suggestions_boundary_detected"])
        self.assertTrue(summary["instagram_list_ambiguous_surface_detected"])
        self.assertTrue(summary["instagram_list_ambiguous_surface_recovery_attempted"])
        self.assertEqual(summary["instagram_list_ambiguous_surface_final_surface"], "followers_exploitable")

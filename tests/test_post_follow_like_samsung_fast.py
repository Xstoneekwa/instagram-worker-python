...
class PreFollowTapInstrumentationTest(unittest.TestCase):
    def _pre_follow_context(
        self,
        username: str = "public_user",
        *,
        followers_list_xml_hint: bool = False,
        private_probe_reused: bool = False,
        private_probe_ms: float = 3.0,
        ...
        ctx = nav.build_pre_follow_tap_context(
            ...
            screen_guard={
                ...
                "followers_list_xml_hint": followers_list_xml_hint,
                "requested": requested,
                "following": following,
            },
            private_gate={
                ...
                "probe_ms": private_probe_ms,
                "probe_reused": private_probe_reused,
        ...
        if stale:
            ctx["captured_at_mono"] = nav.time.monotonic() - 30.0
        return ctx
    ...
    def test_reuse_private_false_recent_skips_second_private_probe(self) -> None:
        device = mock.MagicMock()
        device.click = mock.MagicMock()
        button = self._exact_button()
        logs: list[tuple[str, dict[str, object]]] = []
        ...
        with mock.patch.object(
        ...
        ) as mock_snapshot, mock.patch.object(
            nav, "_follow_state_from_header_button_rid_fast", return_value=("following", {"rid_found": True})
        ), mock.patch.object(
            nav, "verify_app_foreground", return_value=True
        ), mock.patch(
            ...
            out = nav.perform_follow_safe(
                ...
                profile_already_open=True,
                source_profile_username="source_ct",
                visual_candidate_id="vc-reuse-private",
                dont_follow_private_accounts=True,
                pre_follow_context=self._pre_follow_context(
                    "public_user",
                    private_probe_reused=True,
                    private_probe_ms=7.0,
                ),
            )
        ...
        mock_exact.assert_called_once()
        mock_surface.assert_not_called()
        self.assertIn("proof_reuse_private", [event for event, _kw in logs])

    def test_reuse_follow_state_recent_skips_redundant_snapshot_and_runs_exact_probe(self) -> None:
        device = mock.MagicMock()
        device.click = mock.MagicMock()
        button = self._exact_button()
        logs: list[tuple[str, dict[str, object]]] = []
        ...
        with mock.patch.object(
        ...
        ) as mock_exact, mock.patch.object(
            nav,
            ...
            out = nav.perform_follow_safe(
                ...
                profile_already_open=True,
                source_profile_username="source_ct",
                visual_candidate_id="vc-reuse-follow",
                dont_follow_private_accounts=True,
                pre_follow_context=self._pre_follow_context("public_user"),
            )
        ...
        mock_snapshot.assert_not_called()
        mock_exact.assert_called_once()
        self.assertIn("proof_reuse_follow_state", [event for event, _kw in logs])

    def test_pending_requested_reuse_logs_false_requested_following_without_weak_tap(self) -> None:
        device = mock.MagicMock()
        device.click = mock.MagicMock()
        button = self._exact_button()
        logs: list[tuple[str, dict[str, object]]] = []
        ...
        with mock.patch.object(
            ...
            out = nav.perform_follow_safe(
                ...
                profile_already_open=True,
                source_profile_username="source_ct",
                visual_candidate_id="vc-reuse-pending",
                dont_follow_private_accounts=True,
                pre_follow_context=self._pre_follow_context("public_user"),
            )
        ...
        mock_private.assert_not_called()
        mock_exact.assert_called_once()
        self.assertIn("proof_reuse_pending_requested", [event for event, _kw in logs])
        device.click.assert_called_once_with(120, 140)

    def test_stale_pre_follow_context_keeps_normal_probes_before_any_tap(self) -> None:
        device = mock.MagicMock()
        device.click = mock.MagicMock()
        button = self._exact_button()
        logs: list[tuple[str, dict[str, object]]] = []
        ...
        with mock.patch.object(
            ...
            out = nav.perform_follow_safe(
                ...
                profile_already_open=True,
                source_profile_username="source_ct",
                visual_candidate_id="vc-stale",
                dont_follow_private_accounts=True,
                pre_follow_context=self._pre_follow_context("public_user", stale=True),
            )

        self.assertFalse(out["ok"])
        self.assertFalse(out["tapped"])
        device.click.assert_not_called()
        self.assertGreaterEqual(mock_snapshot.call_count, 1)
        mock_private.assert_not_called()
        skipped = [kw for event, kw in logs if event == "proof_reuse_skipped_stale"]
        self.assertTrue(skipped)
        self.assertEqual(skipped[-1]["reason"], "context_stale")

    def test_pre_follow_profile_proof_with_followers_list_hint_uses_exact_bounds_only(self) -> None:
        device = mock.MagicMock()
        device.click = mock.MagicMock()
        button = self._exact_button()
        ...
        with mock.patch.object(
            ...
            out = nav.perform_follow_safe(
                ...
                dont_follow_private_accounts=True,
                pre_follow_context=self._pre_follow_context(
                    "public_user", followers_list_xml_hint=True
                ),
            )
    ...
    def test_pre_follow_profile_proof_with_followers_list_hint_no_bounds_fails_fast_before_surface(self) -> None:
        device = mock.MagicMock()
        device.click = mock.MagicMock()
        ...
        with mock.patch.object(
            ...
            out = nav.perform_follow_safe(
                ...
                profile_already_open=True,
                source_profile_username="source_ct",
                visual_candidate_id="vc-no-bounds",
                dont_follow_private_accounts=True,
                pre_follow_context=self._pre_follow_context(
                    "public_user", followers_list_xml_hint=True
                ),
    ...
    def test_pre_follow_non_reusable_context_still_allows_exact_bounds_probe(self) -> None:
        device = mock.MagicMock()
        device.click = mock.MagicMock()
        button = self._exact_button()
        logs: list[tuple[str, dict[str, object]]] = []
        ...
        with mock.patch.object(
            ...
            out = nav.perform_follow_safe(
                ...
                profile_already_open=True,
                source_profile_username="source_ct",
                visual_candidate_id="vc-805",
                dont_follow_private_accounts=True,
                pre_follow_context=self._pre_follow_context(
                    "public_user",
                    private_probe_reused=True,
                    private_probe_ms=0.0,
                ),
            )
        ...
        mock_exact.assert_called_once()
        mock_surface.assert_not_called()
        self.assertNotIn("pre_follow_tap_context_reused", [event for event, _kw in logs])

    def test_pre_follow_profile_proof_no_bounds_fails_fast_after_exact_probe_without_surface(self) -> None:
        device = mock.MagicMock()
        device.click = mock.MagicMock()
    ...
    def test_exact_follow_probe_accepts_resource_id_match_with_bounds(self) -> None:
        ...
        class Selector:
            def __init__(self, present: bool) -> None:
                ...
                self.info = {
                    "text": "Follow",
                    "contentDescription": "",
                    "resourceName": "com.instagram.android.clone:id/profile_header_follow_button",
                    "bounds": {"left": 40, "top": 120, "right": 240, "bottom": 200},
                }
        ...
        class Device:
            ...
            def __call__(self, **kwargs: object) -> Selector:
                self.calls.append(dict(kwargs))
                return Selector(bool(kwargs.get("resourceIdMatches")))

            def click(self, x: int, y: int) -> None:
                self.clicks.append((int(x), int(y)))

        device = Device()
        probe_logs: list[tuple[str, dict[str, object]]] = []
        with mock.patch.object(
            nav, "_follow_safe_info", side_effect=lambda sel: sel.info
        ), mock.patch.object(
            fae, "log", side_effect=lambda _level, event, **kw: probe_logs.append((str(event), kw))
        ):
            button, meta = fae.try_select_exact_profile_header_follow_fast(
                device,
                nav,
                "com.instagram.android",
                visual_candidate_id="vc-rx",
                ui_snap="follow",
                raw_inv=False,
        ...
        self.assertEqual((meta or {}).get("center_x"), 140)
        self.assertEqual((meta or {}).get("center_y"), 160)
        self.assertTrue(any("resourceIdMatches" in call for call in device.calls))
        attempted = [kw for event, kw in probe_logs if event == "follow_action_exact_probe_attempted"]
        selected = [kw for event, kw in probe_logs if event == "follow_action_exact_follow_fast_path_selected"]
        result = [kw for event, kw in probe_logs if event == "follow_action_exact_probe_selector_result"]
        self.assertTrue(attempted)
        self.assertTrue(selected)
        self.assertTrue(result)
        self.assertEqual(result[-1]["reason"], "resource_id_matches_found")
        self.assertTrue(result[-1]["tap_allowed"])
...
class PostFollowLikeSamsungFastTest(unittest.TestCase):
    ...
    def test_return_ct_candidate_action_bar_never_succeeds_without_ct_proof(self) -> None:
        device = mock.MagicMock()
        logs: list[tuple[str, dict[str, object]]] = []
        stale_candidate_det = {
            "is_followers_list": True,
            "action_bar_title": "caroline.christmann.pro",
        ...
        with mock.patch.object(
            nav,
            "detect_followers_list_screen",
            return_value=stale_candidate_det,
        ) as detect, mock.patch.object(
            nav, "verify_followers_list_surface_is_ct_account", return_value=False
        ), mock.patch(
            "navigation_engine.observe_instagram_state",
            return_value={
                "state": "FOLLOWERS_LIST",
            ...
            nav.time, "monotonic", side_effect=[100.0, 106.0]
        ), mock.patch.object(
            ...
            ok, how, fail = nav.post_follow_controlled_return_to_followers_list(
                device,
                pkg="com.instagram.android",
                source_profile_username="bodyhit_colmar",
                follower_username="caroline.christmann.pro",
                visual_candidate_id="xml_list:caroline.christmann.pro",
                det={},
                max_rounds=1,
                compact_after_follow_verified_mute=True,
                compact_reason="follow_verified_mute_success",
        ...
        self.assertFalse(ok)
        self.assertEqual(how, "compact_abort_no_list_confirmed")
        self.assertEqual(fail, "post_follow_return_ct_compact_abort_no_list_confirmed")
        device.press.assert_called_once_with("back")
        ...
        self.assertNotIn("post_follow_return_ct_success", events)
        self.assertNotIn("post_follow_return_ct_visual_confirmed", events)
        self.assertNotIn(
    ...
    def test_return_ct_ambiguous_initial_list_succeeds_only_after_post_back_ct_proof(self) -> None:
        ...
        stale_candidate_det = {
            "is_followers_list": True,
            "action_bar_title": "luizfernandoribeiro1610",
            "current_screen_guess": "likely_profile",
            "open_detection_method": "own_unified_follow_list",
            "own_unified_followers_list_detected": True,
            "follow_list_username_count": 8,
        ...
        with mock.patch.object(
            ...
            nav, "verify_followers_list_surface_is_ct_account", side_effect=_verify_ct
        ), mock.patch(
            "navigation_engine.observe_instagram_state",
            return_value={
                "state": "FOLLOWERS_LIST",
            ...
            nav.time, "perf_counter", return_value=100.0
        ), mock.patch.object(
            nav.time, "sleep", side_effect=lambda *_a, **_k: None
        ), mock.patch.object(
            nav,
            ...
            ok, how, fail = nav.post_follow_controlled_return_to_followers_list(
                device,
                pkg="com.instagram.android",
                source_profile_username="letournant",
                follower_username="luizfernandoribeiro1610",
                visual_candidate_id="xml_list:luizfernandoribeiro1610",
                det={},
                max_rounds=1,
                compact_after_follow_verified_mute=True,
                compact_reason="follow_verified_mute_success",
        ...
        self.assertIn("post_follow_return_ct_visual_confirmed", events)

    def test_return_ct_compact_initial_strict_ct_proof_succeeds(self) -> None:
        device = mock.MagicMock()
        ct_det = {
            "is_followers_list": True,
            "action_bar_title": "bodyhit_colmar",
            "current_screen_guess": "followers_list",
            "open_detection_method": "own_unified_follow_list",
            "own_unified_followers_list_detected": True,
        }

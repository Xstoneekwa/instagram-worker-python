-- Entry 2E-2A corrective patch — explicit login_status priority over generic verification.
--
-- Remote smoke showed needs_2fa/checkpoint were skipped when
-- provisioning_status='login_verification_pending'. Explicit login statuses must
-- win before generic verification_pending handling.
...
comment on function public.sync_account_dashboard_actions_from_status(
  ...
  'Entry 2E-2A service-role RPC that derives safe account_dashboard_actions from client_instagram_accounts and active account_credentials status. Explicit login_status values (needs_2fa, checkpoint, failed, mismatch, logged_out, connected) take priority over generic verification_pending states.';

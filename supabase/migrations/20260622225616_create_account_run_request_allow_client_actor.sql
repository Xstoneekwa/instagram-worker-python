-- Forward-only: allow client actor attribution for dashboard-initiated Connect enqueue.
-- actor_type remains audit metadata only; RPC execute stays service_role-only.
...
comment on function public.create_account_run_request(uuid, uuid, text, text, text, text, integer, jsonb) is
  'Run Control enqueue RPC. actor_type client is audit-only for dashboard client Connect; execute remains service_role-only.';

alter table public.account_run_requests
  drop constraint if exists account_run_requests_actor_type_check;

alter table public.account_run_requests
  add constraint account_run_requests_actor_type_check
  check (actor_type in ('admin', 'assistant', 'ops', 'system', 'internal', 'client'));
